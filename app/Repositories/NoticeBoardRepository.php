...
class NoticeBoardRepository implements NoticeBoardRepositoryInterface 
{
            ...
            ->addColumn('image', function ($row) {
                ...
                if ($row->user) {
                    ...
                    <div class="d-flex px-2 py-1">
                        <div>
                            <img src="' . $url . '/public/assets/img/notice_board/' . $row->user->photo . '" class="avatar avatar-lg"
                                alt="user1">
                        </div>
                    </div>
                    ';
                    ...
                    <div class="d-flex px-2 py-1">
                        <div>
                            <img src="' . $url . '/public/assets/img/notice_board/" class="avatar avatar-lg"
                                alt="user1">
                        </div>
                    </div>
                    ';
    ...
    {
        try {
            $notice_boardDetail = Student::with('user')->get();
            if (Auth::user()->hasRole('student')) {
                $notice_boardDetail = Student::with('user')->where('user_id', Auth::user()->id)->get();
            }
    ...
    public function store($request) 
    {
        try {
            ...
            //     $directory = public_path('assets/img/notice_board/');
        ...
        try {
            ...
            if (isset($student)) {
                ...
                if ($request->hasFile('photo')) {
                    ...
                    $file = time().'.'.$request->photo->extension();
                    $request->photo->move(public_path('assets/img/notice_board/'), $file);
    ...
    {
        $trashednotice_board = Student::with(['user' => fn($q) => $q->onlyTrashed()])->onlyTrashed()->get();
        try {
            return $this->showTableData($trashednotice_board, $trashed = 'trashed');
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Something went wrong');
        }
    }
