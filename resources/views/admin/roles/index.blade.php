    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-white active" aria-current="page"><span
                    class="text-light">Roles</span></li>
        </ol>
            ...
            <div class="card mb-4">
                <div class="card-header pb-0 d-flex">
                    ...
                    <div class="header-buttons ms-auto text-end">
                        <a href="{{ route('create.role') }}" class="btn btn-primary" target="_blank"><i class="fa fa-plus"></i> Add New</a>
                        ...
                        <div class="main-body">
                            <div class="row">
                                @foreach ($modules as $key => $module)
                                    <div class="col-md-4">
                                        <h6 for="" class="text-uppercase">{{ $module->module }}</h6>
                                        @foreach ($permissions as $key => $permission)
                                            @if ($module->module == $permission->module)
                                                <p><input type="checkbox"> {{ $permission->name }}</p>
                                            @endif
                                        @endforeach
                                        <hr>
                                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.view-role-detail', function() {
            ...
            $.get('roles/' + roleId + '/show', function(data) {
        ...
        $(document).on('click', '.delete-role', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.post('roles/' + roleId + '/delete', {_token: '{{ csrf_token() }}'},function() {
                    });
                    Swal.fire({
                        title: 'Deleted!',
